# -------------------------------------------------------------
# DISTRIBUIDORES APP - STREAMLIT (CSV LOCAL)
# -------------------------------------------------------------
...
# =============================
    ...
    distribuidores = st.sidebar.multiselect("Filtrar Distribuidores", st.session_state.df["Distribuidor"].unique())

    todas_cidades = carregar_todas_cidades()
    cidade_selecionada = st.sidebar.selectbox(
        "Buscar Cidade",
        [""] + todas_cidades,
        index=todas_cidades.index(st.session_state.cidade_busca) + 1 if st.session_state.cidade_busca else 0
    )
    if st.sidebar.button("Limpar busca"):
        st.session_state.cidade_busca = ""
        cidade_selecionada = ""
    ...
    if distribuidores:
        df_filtrado = df_filtrado[df_filtrado["Distribuidor"].isin(distribuidores)]

    if st.session_state.cidade_busca:
        cidade_nome, estado_sigla = st.session_state.cidade_busca.split(" - ")
        df_filtrado = df_filtrado[
            (df_filtrado["Cidade"].str.lower() == cidade_nome.lower()) &
            (df_filtrado["Estado"].str.upper() == estado_sigla.upper())
        ]
        if df_filtrado.empty:
            st.warning(f"❌ Nenhum distribuidor encontrado em **{cidade_nome} - {estado_sigla}**.")
        else:
            st.success(f"✅ {len(df_filtrado)} distribuidor(es) encontrado(s) em **{cidade_nome} - {estado_sigla}**:")
            st.dataframe(df_filtrado[["Distribuidor","Contato","Email","Estado","Cidade"]], use_container_width=True)

    mapa = criar_mapa(df_filtrado)
    st_folium(mapa, width=1600, height=850)
